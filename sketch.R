...
for(k in condloop){
  for(i in cellloop){
    ...
    b = t(apply(matrix(data = rnorm(cond$nitem[k]*4,mean = cond$diffmn[k], sd = cond$diffsd[k]), nrow = cond$nitem[k]), MARGIN = 1, FUN = sort))
    d = cbind(a,b)
    si <- t(apply(d, 1,
                  function(x) traditional2mirt(x = x, 'gpcm', ncat=5)))
    N = paddle$pers
    response = simdata(a = si[,1], d = si[,7:11], N = length(N),Theta = as.matrix(paddle$pers), itemtype = 'gpcm')
